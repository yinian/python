...
from answer.DataBaseManager import DataBaseManager
...
def add_info():
    ...
    if not checker.check_add_fields_exists(info):
        return json.dump({'success':False, 'reason': '字段不完整'},ensure_ascii=False)
    fail_reason = checker.check_value_valid(info)
    if fail_reason:
        return json.dump({'success': False, 'reason':fail_reason},ensure_ascii=False)
    info['deleted'] = 0
    insert_result = manager.add_info(info)
    return json.dump({'success': insert_result})
...
def update_info():
    ...
    if people_id == -1:
        return json.dump({'success':False,'reason':'ID必须为数字'})
